...
const pastMatches = async (userId: number, count: number): Promise<interfaces.IGetMatchObj[]> => {
    const query = 'SELECT id, user_id_1, user_id_2, score, created_at, city, country FROM match WHERE ended=true AND (user_id_1=$1 OR user_id_2=$1) AND deleted_at IS NULL ORDER BY ended_at DESC LIMIT $2';
